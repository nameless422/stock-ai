...
The LLM-assisted strategy generator (`build_strategy_generation_context` / `generate_strategy_code`) supports DeepSeek (`DEEPSEEK_API_KEY`) or OpenAI-compatible (`LLM_API_KEY` / `OPENAI_API_KEY`) endpoints and sends the contract + allowed-field list as part of the prompt.
